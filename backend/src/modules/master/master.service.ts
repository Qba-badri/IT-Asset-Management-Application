...
export class MasterService implements OnModuleInit {
  ...
  private async seedLookups() {
    // Clear existing license categories to ensure clean slate
    ...
    const lookups = [
      ...
      // License Type (Now "Licensing Model")
    ...
    for (const item of lookups) {
      ...
      if (exists) {
        // Update existing to sync labels/sort/description
  ...
  private async seedVendorsAndPlans() {
    const vendors = [
      'Microsoft',
      'AWS',
      'SAP',
    ...
    const plans: Record<string, { name: string; family?: string; type?: string; category?: string }[]> = {
      Microsoft: [
  ...
  async findPlansByVendor(vendorId: number) {
    return this.planRepo.find({
      where: { vendorId: vendorId as any, isActive: true },
  ...
  // Lookups
  async findLookupsByType(type: string) {
